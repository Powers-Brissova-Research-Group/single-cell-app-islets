...
#load data----
# Memoize the dataset load process
load_data <- memoise(function() {
  load("DATA/Islets2.Rda")
  return(Islets)
})

Islets <- load_data()  # Cached dataset loading
  ...
  #* table 1 -Gene Description----
{
  ...
  # use reactive() here so that gene filtering happens once per gene selection
  gene_data <- reactive({
    req(input$Gene)
  ...
  output$table1_vlnplot <- renderTable({
    gene_data()
  })
  output$table1_umap <- renderTable({
    gene_data()
  })
  output$table1_dotplot <- renderTable({
    gene_data()
  })
  output$table1_cellno <- renderTable({
    gene_data()
  })
  ...
  #* plot 4 (umap) ----
  expensive_plot4 <- memoise(function(gene) {
  ...
  output$plot4<- renderPlot({
    req(input$Gene)
    expensive_plot4(input$Gene)  # This result is cached
  }, height = 400, width = 600)
